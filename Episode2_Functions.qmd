---
title: "Episode 2 - Functions and Plots"
format: html
---
...
Here we will show two functions which you would need to run if you are working on your own computer. To run these functions, remove the "\#" (hash symbol), and use command+return or control+enter.

```{r}
# This function will download the package from a remote repository.
# install.packages("palmerpenguins")

# This function loads the package into your current RStudio window
library(palmerpenguins)
```

Functions have a name and are always followed by a set of round brackets. Inside the round brackets is the target of the function. Here, we have used the install.packages function to download and install a package (you can think of a package as being like an app, which usually contains *more* functions and sometimes some example data). The package we have installed is called palmerpenguins, which is a collection of data about penguins, and before we can use it we need to load it into R, which we do with the library() function.

Straight away you'll see that in the first case, we put palmerpenguins in quote marks, and in the second case we didn't use quote marks. We will cover this more later, but for now it's enough to note that we can put a lot of different information into the round brackets.

Now that we've used the library() function to load the palmerpenguin package, we can access the penguin data. The data is stored in an object called penguins, and we can't see it but it's in the background. We can look at it using the View() function, which will display the data in a new window. Take a second to look at the column and row names, and some of the data.
...
Whenever you encounter a function and you don't know what it's doing, you can always ask for help. Type a ? in front of the function and RStudio will open a window with the function manual. These can be dense, but the Description can be informative.
...
One clue comes from colouring the points by sex - we see a repeating pattern from bottom left to top right, which starts with a cluster of females, then a cluster of males, then another cluster of females, and another cluster of males. In many bird species the male is on average heavier than the female (interestingly, the opposite is true in many raptor species), so it is strange to see a sub-cluster of females that are heavier than a sub-cluster of males.
...
```{r}
# These next two lines are installing and loading ggthemes, a package which let's us use the scale_color_colorblind() function. This function will change the colour scheme to something appropriate for colour-blind individuals. 
