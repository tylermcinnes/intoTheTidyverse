...
Here we will calculate a new value, called the winLossRatio, by dividing W by L for each row. Note the slightly different format for mutate(), which is the convention.
...
In the space below, write code required to remove any row where W or L is 0, then use the mutate() function from above to create the new winLossRatio column. Use the head() function to display your results. Feel free to add additional functions and arguments if you think they are useful. 
...
Out of interest, you could ask what the highest ratio is:
